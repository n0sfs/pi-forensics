...
class TestDiagnoseUsbPortHealth:
    def _real_journal_sample(self):
        ...
        return "\n".join([
            "Sep 06 15:45:31 testhost kernel: usb 1-1-port1: unable to enumerate USB device",
            "Sep 06 15:45:32 testhost kernel: usb 1-1.1: device descriptor read/64, error -32",
            "Sep 06 15:45:33 testhost kernel: usb 1-1.1: Device not responding to setup address.",
            "Sep 06 15:45:40 testhost kernel: usb 1-1-port1: attempt power cycle",
            "Sep 06 15:46:00 testhost kernel: usb 1-1.1: device descriptor read/64, error -32",
            "Sep 06 16:44:50 testhost kernel: usb 1-1.1: New USB device found, idVendor=058f, idProduct=6387, bcdDevice= 1.05",
            "Sep 06 16:44:50 testhost kernel: usb-storage 1-1.1:1.0: USB Mass Storage device detected",
            "Sep 06 12:00:00 testhost kernel: usb 1-1.3: New USB device found, idVendor=0781, idProduct=5567, bcdDevice= 1.00",
        ])
    ...
    def test_a_malformed_or_unrelated_line_is_skipped_not_raised(self):
        garbage = "\n".join([
            ...
            "Sep 06 15:45:31 testhost kernel: usb 1-1.1: device descriptor read/64, error -32",
            "",
    ...
    def test_bus2_superspeed_lines_are_never_classified_matching_this_apps_own_disclosed_scope(self):
        ...
        with mock.patch("core.usb_port_health.subprocess.run",
                         return_value=_fake_completed(stdout="Sep 06 15:45:31 testhost kernel: usb 2-1: new SuperSpeed USB device number 2 using xhci_hcd")):
            result = usb_health.diagnose_usb_port_health()
        assert result["available"] is True
        for idx in ("1", "2", "3", "4"):
            assert result["ports"][idx]["failure_count"] == 0
