...
const TOOL_REFERENCE_GROUPS = [
    ...
    },
    {
        ...
        tools: [
            ...
            ["ALEAPP / iLEAPP", "Comprehensive, community-maintained mobile artifact parsers - hundreds of app-specific artifacts (WhatsApp, Signal, Chrome, WiFi history, and more) from an already-acquired Android pull or iOS backup. Right-click the extraction folder and choose \"Parse with ALEAPP/iLEAPP...\". Runs as a background job - large extractions can take several minutes. Whatever it finds is automatically parsed into File Views' Parsed Artifacts index, not just left as a standalone HTML report."],
            ["Android SMS/Contacts/Call Log Parser", "Reads a rooted Physical acquisition's own on-device SMS, contacts, and call log databases straight out of the raw image (this data isn't reachable via a non-rooted Pull/Backup). Right-click the image and choose \"Parse Android SMS/Contacts/Call Log (Whole Image)\"."],
            ["ALEAPP/iLEAPP Location History Export", "Scans everything ALEAPP/iLEAPP has already parsed into the case for plausible GPS coordinates and exports a KML map, viewable in File Explorer and Reporting's Geolocation tab. Right-click any folder and choose \"Export ALEAPP/iLEAPP Location History (KML)\" - requires an active case with at least one prior scan."],
            ["Google Takeout Import", "Imports an already-downloaded Google Takeout export - Search History, YouTube History, Location History, Maps places, and photo metadata into the same searchable index and map view as everything else. Only ever reads a file you already obtained yourself through Google's own export tool; never touches a live account. Right-click the extracted Takeout folder (or a folder holding the downloaded .zip part(s)) and choose \"Import Google Takeout Archive\"."],
