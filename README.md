...
mobile pull/backup or Logical Acquisition folder) with parsed-artifact timestamps (charted by
source, with anti-forensic-indicator flagging), and a station-wide Audit Trail filtered
...
command, e.g.:
```bash
sudo git clone --branch v1.3.0 https://github.com/n0sfs/pi-forensics.git /opt/pi-forensics && cd /opt/pi-forensics && sudo python3 install.py
```
