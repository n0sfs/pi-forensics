...
Recycle Bin, and LNK shortcuts; NTFS `$MFT` (with timestomping detection) and `$UsnJrnl` change-journal
...
command, e.g.:
```bash
sudo git clone --branch v1.15.0 https://github.com/n0sfs/pi-forensics.git /opt/pi-forensics && cd /opt/pi-forensics && sudo python3 install.py
```
