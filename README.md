...
[![Version](https://img.shields.io/badge/version-1.32.0-brightgreen)](CHANGELOG.md)
...
drives, clipboard contents, PowerShell console history, and (when run elevated) Prefetch execution
evidence, with process executables hashed on both platforms for consistency. Both collectors can also acquire a
...
command, e.g.:
```bash
sudo git clone --branch v1.32.0 https://github.com/n0sfs/pi-forensics.git /opt/pi-forensics && cd /opt/pi-forensics && sudo python3 install.py
```
