...
- Bit-stream acquisition: `dc3dd` (`.dd`) and `ewfacquire` (`.E01`) with MD5/SHA-1/SHA-256
- **ddrescue** recovery workflow with staged passes, mapfile audit, and resume support
- Software write-blocker toggle + global USB read-only udev rules (see warnings below)
- SMART health diagnostics before imaging
- SMB/CIFS and NFS discovery & mounting (credentials never placed on process argv)
- Dual-pane file explorer, report editor, hash verification, PDF export
- Production install: dedicated user, gunicorn, **nginx TLS**, scoped sudoers
...
sudo git clone https://github.com/n0sfs/pi-forensics.git /opt/pi-forensics
cd /opt/pi-forensics
