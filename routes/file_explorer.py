...
def parse_crypto_wallets():
    ...
    parsing (see core/crypto_artifacts.py's module docstring for why). Same
    shape as parse_registry()/parse_browser_artifacts() above."""
    req = request.get_json() or {}
    target_dir = safe_path(req.get('path'))
    if not target_dir or not os.path.isdir(target_dir):
        return jsonify({"success": False, "error": "Directory not found or outside the permitted evidence directory."}), 400
    ...
    candidate_paths, truncated = find_crypto_wallet_files(target_dir)
    counts = {}
    files_parsed = 0
    for path in candidate_paths:
        filename = os.path.basename(path)
        records = parse_crypto_wallet_file(path, filename)
        if not records:
            continue
        files_parsed += 1
        for r in records:
    ...
    log_chain_of_custody("crypto_wallets_scanned", {
        "directory": target_dir, "candidates_found": len(candidate_paths),
        "files_parsed": files_parsed, "counts": counts, "truncated": truncated,
    })
    return jsonify({
...
def parse_srum():
    ...
    candidate-file matcher/dispatcher pair (core/srum_utils.py)."""
    req = request.get_json() or {}
    target_dir = safe_path(req.get('path'))
    if not target_dir or not os.path.isdir(target_dir):
        return jsonify({"success": False, "error": "Directory not found or outside the permitted evidence directory."}), 400
    ...
    candidate_paths, truncated = find_srum_files(target_dir)
    counts = {}
    files_parsed = 0
    for path in candidate_paths:
        records = parse_srum_file(path)
        if not records:
            continue
        files_parsed += 1
        for r in records:
    ...
    log_chain_of_custody("srum_files_parsed", {
        "directory": target_dir, "candidates_found": len(candidate_paths),
        "files_parsed": files_parsed, "counts": counts, "truncated": truncated,
    })
    return jsonify({
...
def parse_powershell_history():
    ...
    parse_srum()/parse_prefetch() above, just a different candidate-file
    matcher/dispatcher pair (core/powershell_history_utils.py)."""
    req = request.get_json() or {}
    target_dir = safe_path(req.get('path'))
    if not target_dir or not os.path.isdir(target_dir):
        return jsonify({"success": False, "error": "Directory not found or outside the permitted evidence directory."}), 400
    ...
    candidate_paths, truncated = find_powershell_history_files(target_dir)
    counts = {}
    files_parsed = 0
    for path in candidate_paths:
        records = parse_powershell_history_file(path)
        if not records:
            continue
        files_parsed += 1
        for r in records:
    ...
    log_chain_of_custody("powershell_history_parsed", {
        "directory": target_dir, "candidates_found": len(candidate_paths),
        "files_parsed": files_parsed, "counts": counts, "truncated": truncated,
    })
    return jsonify({
...
def parse_firewall_log():
    ...
    candidate_paths, truncated = find_firewall_log_files(target_dir)
    counts = {}
    files_parsed = 0
    for path in candidate_paths:
        records = parse_firewall_log_file(path)
        if not records:
            continue
        files_parsed += 1
        for r in records:
...
@requires_permission('file_explorer')
def parse_jumplists():
    """Whole-directory scan for Windows Jump List files
    (.automaticDestinations-ms / .customDestinations-ms) - same shape as
    parse_prefetch()/parse_registry() above, just a different candidate-
    file matcher/dispatcher pair (core/jumplist_utils.py)."""
    req = request.get_json() or {}
    target_dir = safe_path(req.get('path'))
    if not target_dir or not os.path.isdir(target_dir):
        return jsonify({"success": False, "error": "Directory not found or outside the permitted evidence directory."}), 400
    ...
    candidate_paths, truncated = find_jumplist_files(target_dir)
    counts = {}
    files_parsed = 0
    for path in candidate_paths:
        records = parse_jumplist_file(path)
        if not records:
            continue
        files_parsed += 1
        for r in records:
    ...
    log_chain_of_custody("jumplists_parsed", {
        "directory": target_dir, "candidates_found": len(candidate_paths),
        "files_parsed": files_parsed, "counts": counts, "truncated": truncated,
    })
    return jsonify({
